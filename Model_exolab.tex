...
There is a continuum of households indexed by $i \in [0,1]$ that chooses how much to consume each period and saves in a bond in zero net supply subject to a soft borrowing penalty. We abstract from two aspects of the household problem: labor and portfolio choice. For labor supply, we assume that households face uninsurable shocks to employment status in the form of an exogenous Poisson process, while we abstract from the portfolio choice problem by assuming that each household receives a fixed and equal share of each intermediate good producer's profits each period. 
...
\begin{align*}
\max_{ \{ c_{it} \}_{t \in \mathbb{N}}} & \quad \expt{0}{ \int_0^{\infty} e^{-\rho t} \left( u( c_{it} ) + \varphi(a) \cdot I_{\{ a \leq \underline{a} \}} \right) dt  } \\[10pt]
\text{subject to} & \quad d \begin{bmatrix}
a_{it} \\
n_{it} 
\end{bmatrix} = \begin{bmatrix}
\tilde{r}_t a_{it} + \tilde{w}_t n_{it} - c_{it} + \tilde{\Pi}_t \\
0 
...
\end{bmatrix} d N_t^i
\end{align*}
where $\{ N_t^i \}_{t \geq 0}$ is a Poisson process with intensity $\lambda(N_t^i)$ that is independent across households and $\varphi(a) \cdot I_{\{ a \leq \underline{a} \}}$ penalizes savings below $\underline{a}$. The corresponding HJBE of the household is given as 
\begin{align*}
\rho V_t( a,n) & = \max_{ c \geq 0} \Bigg[ u(c) + \varphi(a) \cdot I_{\{a \leq \underline{a} \} } + \partial_a V_t(a,n) \left( \tilde{r}_{t}  a + \tilde{w}_t n - c + \tilde{\Pi}_t \right) \\
& \quad + \lambda(n) \left( V_t(a, \check{n}) - V_t( a, n) \right)  \Bigg] + \partial_t V_t(a,n)
\end{align*}
...
\end{align*}
Finally, the real profits ex-adjustment costs of the intermediate good producers are all equal to 
\begin{align*}
...
A symmetric REE in this model is a collection of stochastic processes $\{ c_t(\cdot) g_t(\cdot), P_t, Y_t, q_t = (\mu_{rt}, \sigma_{rt}, w_t, \Pi_t) \}_{i \in [0,1], t \in \mathbb{N}}$ such that: 
\begin{enumerate}[label=\arabic*)]
\item Given the belief $\tilde{q}_t = (\tilde{\mu}_{rt}, \tilde{\sigma}_{rt}, \tilde{w}_t, \tilde{\Pi}_t)$ about the price processes, $\{c_t(\cdot ) \}_{t \in \mathbb{N}}$ is a sequence of consumption policy functions that solve the household's problem, and $g_t(\cdot)$ is the resulting distribution of wealth and labor supply at time $t$. \\
...
\item The market clearing conditions are satisfied for any $t \geq 0$: 
\begin{align*}
 \frac{Y_t}{e^{z_t}} &  = N^* := \sum_{j=1}^2 n_j \int_0^1 g_t(a,n_j) da \tag{Labor Market} \\[10pt]
 dB_t & =r_t B_t dt, \quad B_0 = 0 \quad \Leftrightarrow \quad B_t = 0 \quad \forall t \geq 0  \tag{Bond Market}  \\[10pt]
Y_t & = \sum_{j=1}^2 \int_{-\infty}^{\infty} c_t(a,n) g_t(a,n) da + \frac{\psi}{2} \pi_t^2 Y_t  \tag{Goods Market}
\end{align*}
where $B_t = \sum_{j=1}^2 \int_{-\infty}^{\infty} a \hspace{0.1cm} g_t(a,n_j) da$ \\[10pt]



\item The firms earn zero overall profits: 
\begin{align*}
    Y_t = w_t N^* + \frac{\psi}{2} \pi_t^2 Y_t \\
\end{align*}
...
\begin{align*}
\frac{dY_t}{Y_t} & = \left( \mu_z(z_t)  + \frac{1}{2} \sigma_z^2 \right) dt + \sigma_z dW_t^0 \\[10pt]
\left( \frac{dY_t}{Y_t}  \right)^2 & = \sigma_z^2 dt, 
\end{align*}
so that the NKPC can be written as 
\begin{align*}
d \pi_t = \Bigg[ r^* \pi_t + (\phi_{\pi} -1)\pi_t^2 +  \frac{\epsilon}{\psi} \left( \frac{\epsilon-1}{\epsilon} - \frac{w_t}{e^{z_t}} \right) - \left( \mu_z(z_t) - \frac{1}{2} \sigma_z^2 \right) \pi_t \Bigg] dt - \sigma_z \pi_t dW_t^0. 
\end{align*}
...
where
\begin{align*}
s(a,n, z,\pi, g) & = r(z,\pi,g) a + w(z,\pi,g) n - (u')^{-1} \left[ \partial_a V(a,n,z, \pi,g) \right] \\[10pt]
...
w(z,\pi,g) & = \left( 1 - \frac{\psi}{2} \pi^2 \right) e^z  \\[10pt]
...
& \quad + \varphi'(a) \cdot I_{\{ a \leq \underline{a} \} } \tag{Borrowing Penalty}\\[10pt]
...
& \quad - \sigma_z^2 \pi \partial_{\pi z} V(a,n,z,\pi,g) \tag{Cross Variation Term} \\[10pt]
& \quad + \sum_{j=1}^2 \int_{-\infty}^{\infty} D_g V(a',n_j ; a,n,z,\pi,g) \mu_g( a', n_j ; z,\pi, g) da'  \tag{Wealth Distribution} 
\end{align*}
where 
\begin{align*}
(a',n_j) \mapsto D_g V(a',n_j ; a,n,z,\pi,g)    
\end{align*}
is the Frechet derivative of $V(a,n, z,\pi, g)$ with respect to the distribution $g$ evaluated at the point $(a',n_j)$ on the state space. 
...
\subsection{Finite Agent Approximation}

We now approximate the wealth distribution using a finite number of agents. The approximate state space consists of $a,n$, the household's individual states, $z,\pi$, the finite dimensional macro aggregates, and $\varphi^{-0} = \{ a_i, n_i\}_{1 \leq i \leq N}$, the wealth and labor supply positions of the other agents in the economy. Given our initial condition $B_0 = 0$, we choose $a_N$ so that 
\begin{align*}
